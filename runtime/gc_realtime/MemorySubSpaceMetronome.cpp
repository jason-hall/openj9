...
MM_MemorySubSpaceMetronome::yieldWhenRequested(MM_EnvironmentBase *env)
{
	...
	if (sched->_mode != MM_Scheduler::MUTATOR) {
		MM_JNICriticalRegion::releaseAccess((OMR_VMThread *)env->getOmrVMThread(), &accessMask);
		while (sched->_mode != MM_Scheduler::MUTATOR) {	
			omrthread_sleep(10);
		}
		MM_JNICriticalRegion::reacquireAccess((OMR_VMThread *)env->getOmrVMThread(), accessMask);
	}
}
...
MM_MemorySubSpaceMetronome::collectOnOOM(MM_EnvironmentBase *env, MM_GCCode gcCode, MM_AllocateDescription *allocDescription)
{
	...
	if (sched->isInitialized()) {
		...
		sched->continueGC(envRealtime, OUT_OF_MEMORY_TRIGGER, allocDescription->getBytesRequested(), (OMR_VMThread *)env->getOmrVMThread(), true);
	}
...
MM_MemorySubSpaceMetronome::systemGarbageCollect(MM_EnvironmentBase *env, U_32 gcCode) {
	...
	if (sched->isInitialized()) {
		...
		sched->continueGC(envRealtime, SYSTEM_GC_TRIGGER, 0, (OMR_VMThread *)envRealtime->getOmrVMThread(), J9MMCONSTANT_EXPLICIT_GC_RASDUMP_COMPACT != gcCode);
