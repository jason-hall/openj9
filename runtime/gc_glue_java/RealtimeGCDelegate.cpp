...
MM_RealtimeGCDelegate::initialize(MM_EnvironmentBase *env)
{
	...
	_extensions->accessBarrier = (MM_ObjectAccessBarrier *)_realtimeGC->allocateAccessBarrier(env);
...
{
	const UDATA listCount = MM_GCExtensions::getReferenceObjectListCount(env);
...
{
	const UDATA listCount = MM_GCExtensions::getUnfinalizedObjectListCount(env);
	Assert_MM_true(0 < listCount);
...
{
	const UDATA listCount = MM_GCExtensions::getOwnableSynchronizerObjectListCount(env);
	Assert_MM_true(0 < listCount);
